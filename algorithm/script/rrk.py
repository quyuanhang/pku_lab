...
class Ranking():
    def __init__(self, data, n_features=50, lambda_all=0.01, learning_rate=0.1, beta=None, gama=None):
        ...
        self._beta = beta
        self._gama = gama
        self._match_dict, self._pos_dict = self.load_from_sparse_matrix(data[0])
        self._i_match_dict, self._i_pos_dict = self.load_from_sparse_matrix(data[1])
    ...
    def uniform_user_sampling(self, n_samples):
        ...
        for sgd_user in sgd_users:            
            ...
            i_dis_neg_pool = set(self._i_pos_dict[match_item]) | set(self._i_match_dict[match_item])
            i_neg_user = np.random.randint(self._n_users)
            while i_neg_user in i_dis_neg_pool:
                i_neg_user = np.random.randint(self._n_users)
            # 写入样本列表
            samples.append([sgd_user, match_item, pos_item, neg_item, i_pos_user, i_neg_user])
